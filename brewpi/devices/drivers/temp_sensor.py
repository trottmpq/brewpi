...
try: 
    ...
    class TempSensorDriver():
        ...
        def setup(gpio, activeLow):
            ...
            TempSensorDriver.write_spi(gpio, TempSensorDriver.REG_CONFIG, [TempSensorDriver.CONFIG], activeLow) #set up device
            r = TempSensorDriver.read_spi(gpio, TempSensorDriver.REG_CONFIG, 1, activeLow)[0] #read config back
        ...
        def getTemp_degc(gpio, activeLow=True):
            ...
            r = TempSensorDriver.read_spi(gpio, TempSensorDriver.REG_RTD_MSB, 2, activeLow)
            rtdval = (r[0] * 256 + r[1])
            if (rtdval % 2) == 1: #lowest bit is a fault flag
                r = TempSensorDriver.read_spi(gpio, TempSensorDriver.REG_FLT_STATUS, 1, activeLow)[0]
                print("Error 0x{:02x} detected".format(r))
                TempSensorDriver.write_spi(gpio, TempSensorDriver.REG_CONFIG, [TempSensorDriver.CONFIG + 2], activeLow) #try clear fault
...
class pt100():
    ...
    resistance_vals = [ # 18.52 Ohm correspond to a temperature of -200 deg C
        18.52, 18.96, 19.39, 19.82, 20.25, 20.68, 21.11, 21.54, 21.97, 22.40, 
        ...
        390.48] # 390.48 Ohm Pt100 resistance at 850 deg C
    def interp_resist_to_temp(resist):
        try:
            import numpy as np
            return np.interp(resist, pt100.resistance_vals, pt100.temperature_vals)
        except:
            if resist < pt100.resistance_vals[0] or resist > pt100.resistance_vals[-1]: raise ValueError("Value out of range")
            i = 0
