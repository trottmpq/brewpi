...
class TestHeater:
    ...
    def test_is_empty(self, testapp):
        ...
        response = testapp.get("/api/devices/Heater/")
    ...
    def test_create(self, testapp):
        ...
        response = testapp.post_json(
            "/api/devices/Heater/", dict(name="Heater1", gpio_num=1)
        )
        assert response.status_int == 200
        assert response.content_type == "application/json"
        assert response.json == {
    ...
    def test_get_list(self, testapp):
        ...
        testapp.post_json("/api/devices/Heater/", dict(name="Heater1", gpio_num=1))
        testapp.post_json("/api/devices/Heater/", dict(name="Heater2", gpio_num=2))
        response = testapp.get("/api/devices/Heater/")
        assert response.status_int == 200
        assert response.content_type == "application/json"
        assert response.json == [
